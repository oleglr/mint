...
Mintlify is currently in private beta. [Sign up](https://1tc7vihvbit.typeform.com/to/pZJ31XZB) on our waitlist to get access.
...
Every Mintlify deployment must have a `mint.config.json` file with the core configuration settings added. You can learn more about the structure of the config file at the [customization guide](/settings/customization).

### Using the Mintlify CLI

To create a starter configuration file, use the following command.
...
<Tip>
  You can also use a private repo, but it would disable user edit suggestions.
</Tip>
